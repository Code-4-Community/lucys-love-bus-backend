...
public class CheckoutProcessorImpl implements ICheckoutProcessor {
    ...
    public void handleStripeCheckoutEventComplete(String payload, String sigHeader) {
        try {
            Event event = Webhook.constructEvent(
                    payload, sigHeader, this.stripeWebhookSigningSecret
            );
     ...
     * @param checkoutSessionId A checkoutSessionId to associate with registrations which require payment
     */
    ...
    /**
     * For ADMIN and PF users, their Event Registrations are initially Active, because they don't have to
     * pay for events. For GP users, the Event Registration is marked as PAYMENT_INCOMPLETE until we
     * receive a checkout.session.completed event from Stripe in our webhook.
