...
public class RequestsProcessorImpl implements IRequestsProcessor {
  ...
  public void createRequest(CreateRequest requestData, JWTData userData) {
    ...
    List<RequestStatus> pastRequests = db.selectFrom(PF_REQUESTS)
  ...
  public List<RequestData> getRequests(JWTData userData) {
    ...
    List<RequestData> outstandingRequests = db
        .select(PF_REQUESTS.ID, PF_REQUESTS.DESCRIPTION, USERS.EMAIL)
  ...
  public void approveRequest(int requestId, JWTData userData) {
    ...
    PfRequestsRecord requestsRecord = db.selectFrom(PF_REQUESTS)
  ...
  public RequestStatus geRequestStatus(int requestId, JWTData userData) {
    ...
    PfRequests request = db.selectFrom(PF_REQUESTS)
        .where(PF_REQUESTS.ID.eq(requestId))
        .fetchOneInto(PfRequests.class);

    if (userData.getPrivilegeLevel() != PrivilegeLevel.ADMIN) {
      Users requestUser = db.selectFrom(USERS)
